...
class GraphNet(torch.nn.Module):
    ...
    def __init__(self, actions, num_feat, num_label, drop_out=0.3, multi_label=False, batch_normal=False,
        ...
        self.evalate_actions(actions, state_num)
    ...
    def build_model(self, actions, batch_normal, drop_out, num_feat, num_label, state_num):
        ...
        self.build_hidden_layers(actions, num_feat, batch_normal, drop_out, self.layer_nums, num_feat, num_label, state_num)
    ...
    def evalate_actions(self, actions, state_num):
        ...
        if not self.residual:
            reason_length = self.layer_nums * state_num
        else:
            reason_length = sum([state_num+i for i in range(1,self.layer_nums+1)])
        if state_length != reason_length:
            raise RuntimeError("Wrong Input: unmatchable input")
        if actions[-1] == self.num_label:
            pass
        else:
